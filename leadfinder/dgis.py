...
# Оба предела жёсткие, оба проверены живым ответом: больше — ошибка 400.
# Вместе они дают потолок в 50 организаций на один поисковый запрос,
# поэтому охват набирается числом формулировок, а не глубиной выдачи.
PAGE_SIZE = 10          # максимум элементов на странице
PAGE_LIMIT = 5          # максимальный номер страницы
MAX_PAGES = PAGE_LIMIT
REQUEST_PAUSE = 0.35    # пауза между страницами, чтобы не ловить лимиты

# Сортировка по алфавиту — не косметика, а условие работоспособности.
# По умолчанию API сортирует по релевантности и первыми отдаёт лучшие
# карточки: на живой проверке 44 из 50 парикмахерских Уфы имели 5.0,
# и в рабочий диапазон 3.0–4.2 не попадал никто. Дотянуться до остальных
# нельзя — дальше пятой страницы API не пускает. Имя с рейтингом никак
# не связано, поэтому та же выборка становится честной: при сортировке
# по алфавиту в диапазон попало 7 карточек из 50.
#
# Сортировки по возрастанию рейтинга в API нет: значения ограничены
# списком relevance, rating, flamp_rating, distance, distance_without_ads,
...
class DgisClient:
    ...
    def search(self, query: str, region_id: str, max_pages: int = MAX_PAGES) -> Iterator[dict[str, Any]]:
        ...
        # Просить больше пятой страницы бессмысленно: API отвечает 400
        # и роняет весь поиск, а не просто отдаёт меньше
        max_pages = max(1, min(max_pages, PAGE_LIMIT))
