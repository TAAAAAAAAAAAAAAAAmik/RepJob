...
def score_company(company: Company, weigh_contacts: bool = True) -> Company:
    """Проставляет score (0-100), verdict и reviews_needed. Меняет объект на месте.

    weigh_contacts=False, когда телефонов нет ни у кого в выборке: демо-ключ
    2GIS контакты не отдаёт, и штраф в этом случае одинаково просаживает всех,
    только перекрашивая горячие лиды в холодные.
    """
    ...
    contact = 1.0 if (company.phone or not weigh_contacts) else 0.8
...
def rank(companies: list[Company]) -> list[Company]:
    ...
    weigh_contacts = any(c.phone for c in companies)

    for company in companies:
        score_company(company, weigh_contacts=weigh_contacts)
